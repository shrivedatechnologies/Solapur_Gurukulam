...
import {
    Menu, Bell, Moon, Sun, LayoutDashboard, BookOpen, Mic, Library,
    Home, Settings, User, TrendingUp, Shield, LogOut, X,
    Layers
} from 'lucide-react';
...
const DashboardLayout = () => {
    ...
    const getMenuItems = () => {
        ...
        const allItems = [
            { name: 'Dashboard', path: '/dashboard', icon: LayoutDashboard, roles: ['user', 'admin', 'super_admin'] },
            { divider: true },
            { name: '━━━ CONTENT ━━━', path: null, icon: null, roles: ['admin', 'super_admin'], isHeader: true },
            { name: 'Manage Mantras', path: '/dashboard/mantras', icon: Mic, roles: ['admin', 'super_admin'] },
            { name: 'Manage Shlokas', path: '/dashboard/shlokas', icon: BookOpen, roles: ['admin', 'super_admin'] },
            { name: 'Manage Shotrams', path: '/dashboard/shotrams', icon: Layers, roles: ['admin', 'super_admin'] },
            { name: 'Manage Categories', path: '/dashboard/categories', icon: Library, roles: ['super_admin'] },
            { divider: true },
            { name: '━━━ HOMEPAGE ━━━', path: null, icon: null, roles: ['admin', 'super_admin'], isHeader: true },
            { name: 'Manage Homepage', path: '/dashboard/homepage', icon: Home, roles: ['admin', 'super_admin'] },
            { name: 'Analytics', path: '/dashboard/analytics', icon: TrendingUp, roles: ['admin', 'super_admin'] },
            { divider: true },
            { name: '━━━ ADMIN ━━━', path: null, icon: null, roles: ['super_admin'], isHeader: true },
            { name: 'Manage Admins', path: '/dashboard/admins', icon: Shield, roles: ['super_admin'] },
            { divider: true },
            { name: '━━━ ACCOUNT ━━━', path: null, icon: null, roles: ['user', 'admin', 'super_admin'], isHeader: true },
            { name: 'Profile', path: '/dashboard/profile', icon: User, roles: ['user', 'admin', 'super_admin'] },
            { name: 'Settings', path: '/dashboard/settings', icon: Settings, roles: ['user', 'admin', 'super_admin'] },
        ];
    ...
    const linkVariants = {
        hover: { scale: 1.02, x: 4, transition: { type: 'spring', stiffness: 300, damping: 15 } },
        tap: { scale: 0.98 }
    };
    ...
    const SidebarContent = ({ onClose }) => (
        <div className="flex flex-col h-full bg-white/95 dark:bg-gray-800/95 backdrop-blur-md border-r border-amber-200/50 dark:border-gray-700">
            ...
            <div className="flex items-center justify-between p-4 border-b border-amber-100 dark:border-gray-700">
                ...
                {onClose && (
                    <button onClick={onClose} className="lg:hidden p-1 rounded-lg hover:bg-amber-50 transition">
                ...
                <div className="flex items-center space-x-3">
                    ...
                    <div>
                        ...
                        <p className="text-xs text-gray-500 dark:text-gray-400 capitalize">
                            {user?.role === 'super_admin' ? '🔱 Super Admin' : user?.role === 'admin' ? '👑 Admin' : '👤 Devotee'}
                        </p>
                    </div>
                </div>
            </div>
            ...
            <nav className="flex-1 p-4 overflow-y-auto">
                <ul className="space-y-1">
                    {menuItems.map((item, index) => {
                        if (item.divider) {
                            return <li key={`divider-${index}`} className="my-2 border-t border-amber-200/50 dark:border-gray-700"></li>;
                        }
                        if (item.isHeader) {
                            return (
                                <li key={item.name} className="px-3 py-2">
                                    <p className="text-xs font-semibold text-amber-500 uppercase tracking-wider">{item.name}</p>
                                </li>
                            );
                        }
                        ...
                        <div className="flex justify-between items-center h-16">
                            ...
                            <div className="flex items-center space-x-3">
                                ...
                                <motion.button
                                    ...
                                    whileHover="hover"
                                    whileTap="tap"
                                    className="p-2 rounded-lg bg-amber-50 dark:bg-gray-700 hover:bg-amber-100 dark:hover:bg-gray-600 transition-colors relative"
                                >
