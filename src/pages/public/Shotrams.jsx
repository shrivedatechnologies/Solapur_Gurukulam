...
const Shotrams = () => {
  ...
  const { data: shotramResponse, isLoading } = useQuery({
    ...
    queryFn: () => selectedCategory ? shotramApi.getByCategory(selectedCategory, { limit: 200 }) : shotramApi.getAll({ limit: 200 }),
  });

  const allShotrams = Array.isArray(shotramResponse?.data) ? shotramResponse.data : Array.isArray(shotramResponse) ? shotramResponse : [];
  const shotrams = allShotrams.filter(s => s.name?.toLowerCase().includes(searchTerm.toLowerCase()) || s.sanskrit?.toLowerCase().includes(searchTerm.toLowerCase()) || s.meaning?.toLowerCase().includes(searchTerm.toLowerCase()));
    ...
    <div className="min-h-screen bg-gradient-to-b from-[#FDFAF5] to-[#FFFDF7] dark:from-gray-950 dark:to-gray-900 overflow-x-hidden">
      <section ref={heroRef} className="relative overflow-hidden pt-16 pb-20 md:pt-20 md:pb-28">
        ...
        <div className="absolute inset-0 overflow-hidden pointer-events-none">
          {[...Array(8)].map((_, i) => (<motion.div key={i} className="absolute w-1 h-1 rounded-full bg-amber-400/30" style={{ left: `${Math.random() * 100}%`, top: `${Math.random() * 100}%` }} animate={{ y: [0, -30, 0], opacity: [0, 0.5, 0] }} transition={{ duration: 4 + Math.random() * 4, repeat: Infinity, delay: Math.random() * 4 }} />))}
        </div>
        ...
        <motion.div style={{ y: yParallax }} className="relative z-10 max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ type: 'spring', bounce: 0.3 }} className="inline-flex items-center justify-center w-24 h-24 rounded-full bg-gradient-to-br from-amber-100 to-amber-200 shadow-xl mb-6">
            <motion.span animate={{ scale: [1, 1.08, 1], textShadow: ["0px 0px 0px #E8890A", "0px 0px 12px #E8890A", "0px 0px 0px #E8890A"] }} transition={{ duration: 2.5, repeat: Infinity, ease: "easeInOut" }}><Flame className="h-10 w-10 text-amber-700" /></motion.span>
          </motion.div>
          <motion.h1 initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="text-4xl md:text-5xl lg:text-6xl font-bold bg-gradient-to-r from-gray-900 to-gray-700 dark:from-white dark:to-gray-300 bg-clip-text text-transparent">Sacred Shotrams</motion.h1>
          <motion.p initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="text-gray-600 dark:text-gray-300 text-base md:text-lg max-w-2xl mx-auto mt-4 leading-relaxed">Devotional hymns in praise of deities – chanting brings divine grace.</motion.p>
        </motion.div>
        ...
        {!isLoading && (
          <div className="flex flex-wrap items-center justify-between gap-3 mb-8">
            <div className="flex items-center gap-2 text-sm text-gray-500 dark:text-gray-400 bg-white/50 dark:bg-gray-800/50 px-4 py-1.5 rounded-full">
              ...
              <span>{shotrams.length} sacred {shotrams.length === 1 ? 'shotram' : 'shotrams'}</span>
        ...
        {isLoading ? (
          ...
          <motion.div variants={containerVariants} initial="hidden" animate="visible" className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {shotrams.map((shotram) => (
              <motion.div key={shotram._id} variants={itemVariants} whileHover={{ y: -5 }} transition={{ type: 'spring', stiffness: 200, damping: 15 }} className="group">
                  ...
                  <div className="relative h-full bg-white dark:bg-gray-800 rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 border border-amber-200/40">
                    ...
                    <div className="p-5 pb-6">
                      <div className="flex items-start gap-3 mb-3">
                        <div className="w-10 h-10 rounded-full bg-gradient-to-br from-amber-100 to-amber-200 flex items-center justify-center shadow-inner flex-shrink-0"><Flame className="h-5 w-5 text-amber-700" /></div>
                        <div className="flex-1">
                          <h3 className="text-xl font-bold text-gray-900 dark:text-white group-hover:text-amber-600 transition-colors">{shotram.name}</h3>
                          {shotram.category?.name && (<span className="inline-block mt-1 text-xs font-semibold bg-amber-50 dark:bg-amber-900/30 text-amber-700 dark:text-amber-300 rounded-full px-2 py-0.5">{shotram.category.name}</span>)}
                        </div>
                      </div>
                      {shotram.sanskrit && (<div className="mt-3 font-devanagari text-sm text-gray-600 dark:text-gray-300 line-clamp-3 bg-amber-50/40 dark:bg-amber-900/10 rounded-xl px-3 py-2 leading-relaxed">{shotram.sanskrit.slice(0, 180)}</div>)}
                      {shotram.meaning && (<p className="mt-3 text-gray-600 dark:text-gray-400 text-sm line-clamp-2 leading-relaxed">{shotram.meaning.slice(0, 100)}</p>)}
                      <div className="mt-4 flex flex-wrap items-center gap-2 pt-2 border-t border-gray-100 dark:border-gray-700">
                        ...
                        {shotram.kannada && <span className="text-[11px] px-2 py-0.5 rounded-full bg-purple-50 text-purple-600 dark:bg-purple-900/30 dark:text-purple-300">ಕನ್ನಡ</span>}
                        {shotram.marathi && <span className="text-[11px] px-2 py-0.5 rounded-full bg-green-50 text-green-600 dark:bg-green-900/30 dark:text-green-300">मराठी</span>}
                        {shotram.tamil && <span className="text-[11px] px-2 py-0.5 rounded-full bg-red-50 text-red-600 dark:bg-red-900/30 dark:text-red-300">தமிழ்</span>}
                        {shotram.hindi && <span className="text-[11px] px-2 py-0.5 rounded-full bg-blue-50 text-blue-600 dark:bg-blue-900/30 dark:text-blue-300">हिन्दी</span>}
                        {shotram.english && <span className="text-[11px] px-2 py-0.5 rounded-full bg-gray-100 text-gray-600 dark:bg-gray-700 dark:text-gray-300">English</span>}
                        <div className="flex-1" />
                        <span className="flex items-center gap-1 text-xs text-gray-400">👁 {shotram.views?.toLocaleString() || 0}</span>
                      </div>
                      <div className="mt-4 flex justify-end">
                        <span className="text-amber-600 text-sm font-medium flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-all duration-300 group-hover:translate-x-1">Explore <Heart className="h-4 w-4" /></span>
                      </div>
                    </div>
                  </div>
                </Link>
