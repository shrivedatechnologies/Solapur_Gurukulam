...
const ShlokaDetails = () => {
  ...
  // ─── Fetch category data to get image ───
  const categoryId = shloka?.category?._id || shloka?.category;
  const { data: category } = useQuery({
    queryKey: ["category", categoryId],
    queryFn: () => categoryApi.getById?.(categoryId) || categoryApi.getBySlug?.(categoryId),
    enabled: !!categoryId,
  });
  ...
  const getLanguageContent = () => {
    if (!shloka) return { text: "", label: "", icon: "", isSanskrit: true };
    ...
    const content = shloka[currentLanguage];
    if (content) {
      return {
        text: content,
  ...
  const categoryName = category?.name || shloka.category?.name;
